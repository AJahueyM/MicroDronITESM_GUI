...
JoystickAxisExpo::JoystickAxisExpo(double expoValue, double deadZone) {
    ...
    if(this->deadZone < 0 || this->deadZone > MAX_VALUE){
        throw std::runtime_error("Invalid deadZone given to JoystickAxisExpo, needs to be between 0 and " + std::to_string(MAX_VALUE));
    }

    if(this->expoValue < 0 || this->expoValue > MAX_VALUE){
        throw std::runtime_error("Invalid expoValue given to JoystickAxisExpo, needs to be between 0 and " + std::to_string(MAX_VALUE));
    }
